...
def main():
    universe=load_json(DATA/"universe.json",[]);scanner=load_json(DATA/"scanner.json",{});state=load_json(DATA/"state.json",{"active_trades":{},"closed_trades":[]})
    active=state.get("active_trades",{});closed=state.get("closed_trades",[]);by_symbol={x["symbol"]:x for x in universe if x.get("symbol")}
    scanner_assets={x.get("symbol"):x for x in scanner.get("assets",[]) if x.get("symbol")}

    watch=set(active)
    for a in scanner_assets.values():
        if a.get("status")!="ok" or a.get("active"):continue
        score=a.get("near_signal_score")
        if score is None:score=(a.get("candidate") or {}).get("signal_score")
        if score is not None and float(score)>=NEAR_SIGNAL_FLOOR:watch.add(a.get("symbol"))
    watch.discard(None)
    ...
    d=yf.download(tickers,period=CONFIG["history_period"],interval="1d",group_by="ticker",auto_adjust=False,threads=True,progress=False);multi=len(tickers)>1
    black=str(date.today()) in set(load_json(DATA/"macro_blackouts.json",{"dates":[]}).get("dates",[]));exits=[];signals=[];rows=[]
