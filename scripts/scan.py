...
def main():
    ...
    nxt=(start+len(items))%len(u);closed=closed[-500:];save_json(DATA/"cursor.json",{"next_index":nxt,"last_run":now_iso()});save_json(DATA/"state.json",{"active_trades":active,"closed_trades":closed,"updated_at":now_iso()})
    ...
    stats={"universe":len(u),"known_assets":len(assets),"active_trades":len(active),"signals_this_batch":len(signals),"exits_this_batch":len(exits),"batch_processed":proc,"batch_errors":errs,"next_index":nxt,"macro_blackout_today":black,"near_signals":sum(1 for a in assets.values() if a.get("near_signal_score",0)>=60 and not a.get("active")),"completed_trades":completed,"successful_trades":wins,"signal_success_rate_pct":success}
    save_json(DATA/"scanner.json",{"generated_at":now_iso(),"stats":stats,"signals":signals,"exits":exits,"active_trades":sorted(active.values(),key=lambda x:x["symbol"]),"completed_trades":closed,"assets":sorted(assets.values(),key=lambda x:(0 if x.get("active") else 1,x.get("symbol","")))})
    print(stats)
