...
def emit(kind,p):
 email(f"ASX {kind}: {p.get('symbol')} {p.get('strategy','')}",p);url=os.getenv("ALERT_WEBHOOK_URL","").strip()
...
def main():
 ...
 b=load_json(DATA/"macro_blackouts.json",{"dates":[]});black=str(date.today()) in set(b.get("dates",[]));start=int(cur.get("next_index",0))%len(u);items=[u[(start+i)%len(u)] for i in range(min(CONFIG["universe_batch_size"],len(u)))];ts=[x["ticker"] for x in items]
 d=yf.download(ts,period=CONFIG["history_period"],interval="1d",group_by="ticker",auto_adjust=False,threads=True,progress=False);multi=len(ts)>1;signals=[];exits=[];errs=proc=0
 for item in items:
  sym=item["symbol"];x0=hist(d,item["ticker"],multi)
  if len(x0)<CONFIG["required_history_days"]:assets[sym]={"symbol":sym,"company":item["company"],"status":"error","reason":f"Only {len(x0)} bars"};errs+=1;continue
  x=compute(x0).dropna(subset=["ATR14","ADX14","EMA200","ZS20","BBWIDTH"]);r=x.iloc[-1];proc+=1;rg,meta=regime(x,CONFIG);ms=micro(r)
  if sym in active:
   why,px=exit_check(active[sym],x)
   if why:
    tr=active.pop(sym);pnl=((px/tr["entry_price"])-1)*100;payload={**tr,"exit_date":str(x.index[-1].date()),"exit_price":px,"exit_reason":why,"pnl_pct":pnl};exits.append(payload);closed.append(payload);emit("EXIT",payload)
  ...
  assets[sym]={"symbol":sym,"company":item["company"],"status":"ok","regime":rg,"price":float(r.Close),"adx14":float(r.ADX14),"atr14":float(r.ATR14),"relative_volume":float(r.RELVOL20) if pd.notna(r.RELVOL20) else None,"zscore20":float(r.ZS20),"microstructure_score":round(ms,1),"active":sym in active,"candidate":cand,"updated_at":now_iso()}
 nxt=(start+len(items))%len(u);save_json(DATA/"cursor.json",{"next_index":nxt,"last_run":now_iso()});save_json(DATA/"state.json",{"active_trades":active,"closed_trades":closed[-500:],"updated_at":now_iso()});stats={"universe":len(u),"known_assets":len(assets),"active_trades":len(active),"signals_this_batch":len(signals),"exits_this_batch":len(exits),"batch_processed":proc,"batch_errors":errs,"next_index":nxt,"macro_blackout_today":black};save_json(DATA/"scanner.json",{"generated_at":now_iso(),"stats":stats,"signals":signals,"exits":exits,"active_trades":sorted(active.values(),key=lambda x:x["symbol"]),"assets":sorted(assets.values(),key=lambda x:(0 if x.get("active") else 1,x.get("symbol","")))});print(stats)
